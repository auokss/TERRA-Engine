...
Type
  ...
  ALreverbState = Record
    //DERIVE_FROM_TYPE(ALeffectState);

    // All delay lines are allocated as a single buffer to reduce memory
    // fragmentation and management code.
    SampleBuffer:PSingleArray;
    TotalSamples:Cardinal;

    // Master effect filters
    LpFilter:AudioFilterState;
    HpFilter:AudioFilterState; // EAX only
    ...
    DelayTap:Array[0..1] Of Cardinal;

    Early:AudioEarlyReflection;

    // Decorrelator delay line.
    Decorrelator:DelayLine ;

    // There are actually 4 decorrelator taps, but the first occurs at the initial sample.
    DecoTap:Array[0..2] Of Cardinal;

    Late:AudioLateReverb;
...
static inline ALvoid VerbPass(ALreverbState *State, ALfloat in, ALfloat *restrict out)
{
    ...
    in = AudioFilterState_processSingle(&State->LpFilter, in);
...
static inline ALvoid EAXVerbPass(ALreverbState *State, ALfloat in, ALfloat *restrict early, ALfloat *restrict late)
{
    ...
    in = AudioFilterState_processSingle(&State->LpFilter, in);
    in = AudioFilterState_processSingle(&State->HpFilter, in);
...
static ALvoid ALreverbState_update(ALreverbState *State, ALCdevice *Device, const ALeffectslot *Slot)
{
    ...
    // Calculate the master low-pass filter (from the master effect HF gain).
        ...
        AudioFilterState_setParams(&State->LpFilter, ALfilterType_HighShelf,
                                Slot->EffectProps.Reverb.GainHF,
                                hfscale, 0.0f);
        lfscale = Slot->EffectProps.Reverb.LFReference / frequency;
        AudioFilterState_setParams(&State->HpFilter, ALfilterType_LowShelf,
...
static ALeffectState *ALreverbStateFactory_create(ALreverbStateFactory* UNUSED(factory))
{
    ...
    AudioFilterState_clear(&state->LpFilter);
    AudioFilterState_clear(&state->HpFilter);
